...
class SentryTest extends \PHPUnit_Framework_TestCase {
	...
	public function test_MessageAndContext_Log_ShouldCallCaptureMessageWithContextAsExtra() {
		$this->expectsCaptureMessageToBeCalledWith(self::A_MESSAGE, ['level' => \Raven_Client::DEBUG, 'extra' => [self::CONTEXT_KEY => self::CONTEXT_VALUE]]);
	...
	public function test_ContextWithExceptionAndOtherKeys_Log_ShouldCaptureExceptionWithContextAsExtraWithoutException() {
		$exception = new \Exception(self::A_MESSAGE);
		$this->expectsCaptureExceptionToBeCalledWith($exception, ['level' => \Raven_Client::DEBUG, 'extra' => [self::CONTEXT_KEY => self::CONTEXT_VALUE]]);

		$this->writer->log(LogLevel::DEBUG, self::A_MESSAGE, [Logger::EXCEPTION_CONTEXT => $exception, self::CONTEXT_KEY => self::CONTEXT_VALUE]);
	}
